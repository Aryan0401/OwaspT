...
void main() {
  ...
  vec2 p = (gl_FragCoord.xy - (0.5 + uCenterOffset) * uResolution.xy) * px;
...
export default function MagicRings({
  ...
  propsRef.current = {
    ...
    hoverScale, parallax, clickBurst, alphaMode, centerOffset, targetSelector,
  };
  ...
  useEffect(() => {
    ...
    const animate = (t) => {
      ...
      let offsetX = p.centerOffset[0];
      let offsetY = p.centerOffset[1];

      if (p.targetSelector) {
        const target = document.querySelector(p.targetSelector);
        if (target && mount) {
          const mountRect = mount.getBoundingClientRect();
          const targetRect = target.getBoundingClientRect();
          if (mountRect.width > 0 && mountRect.height > 0) {
            offsetX = (targetRect.left + targetRect.width * 0.5 - mountRect.left) / mountRect.width - 0.5 + p.centerOffset[0];
            offsetY = 0.5 - (targetRect.top + targetRect.height * 0.5 - mountRect.top) / mountRect.height + p.centerOffset[1];
          }
        }
      }

      uniforms.uCenterOffset.value.set(offsetX, offsetY);
